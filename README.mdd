...
def delete_account():
    ...
    choice = input("Qaysini o‘chirmoqchisiz?: ").strip()
    if not choice.isdigit() or int(choice) < 1 or int(choice) > len(sessions):
        print("❌ Noto‘g‘ri tanlov")
        input("Enter bosing...")
        return

    session_file = os.path.join(SESSIONS_DIR, sessions[int(choice)-1] + ".session")
    if os.path.exists(session_file):
        os.remove(session_file)
        print("✅ Akkaunt o‘chirildi")
    input("Enter bosing...")
